...
5. 收藏成功后，网页右上角和插件弹窗都会显示「已加入 Later Space」，并提供「查看」和短时「撤销」入口；「查看」会直接定位到刚加入的卡片。
6. 正式页面暂时不可用时，插件会在本地排队，并每分钟自动重试。
7. 插件会复用已经打开的 Later Space 页面；未打开时会在后台短暂打开正式页面完成本地入库。
