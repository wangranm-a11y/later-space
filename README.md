...
5. 服务暂时不可用时，插件会在本地排队，并每分钟自动重试。
6. 若服务地址或令牌不同，在插件详情中打开「扩展程序选项」修改并测试连接。
...
GitHub Pages 和当前 Vercel 部署都是静态网站，不提供 `/api/inbox`。要实现离开家、Mac 休眠后仍自动收集，需要把收件箱 API 单独部署到公网并加入用户认证；插件与 iOS App 的服务地址已经设计为可切换，无需重写入口。
