...
一个本地优先的无限收藏画布。文字、图片、视频、网址与社媒分享文案使用同一个入口，也可以直接粘贴到画布；默认显示低视觉噪音的「链接与文字」，并可切换到「素材」或「全部」。链接、文字、图片和视频仍可在筛选面板中进一步细分。所有文字都会在本地自动提取临时标题并收成纸质信息卡片，双击可在封面与可缩放的全文之间原地切换；链接会提取标题并生成极简文字海报，可从海报或选中工具栏打开原内容。
