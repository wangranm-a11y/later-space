...
class PwaAuthHandoffContractTest(unittest.TestCase):
    ...
    def test_browser_uses_short_lived_cookie_not_session_tokens(self):
        ...
        self.assertIn('PWA_HANDOFF_STORAGE_KEY = "later-space-pwa-handoff-v1"', APP)
        self.assertIn("localStorage.setItem(PWA_HANDOFF_STORAGE_KEY", APP)
        self.assertIn("localStorage.removeItem(PWA_HANDOFF_STORAGE_KEY)", APP)
        self.assertIn('handoffUrl.searchParams.set("pwa_handoff", handoffCode)', APP)
        self.assertIn('new URLSearchParams(location.search).get("pwa_handoff")', APP)
        self.assertIn('"pwa_handoff"].forEach((key)', APP)
        self.assertIn("signal: controller.signal", APP)
        self.assertIn("登录状态恢复超时，请重新打开 Later Space", APP)
