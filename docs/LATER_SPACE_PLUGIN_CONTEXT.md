...
- 正式安装包：`dist/later-space-chrome-extension-v1.9.3.zip`
...
- `1.9.3` 使用版本化 zip 和固定的包内目录，避免覆盖升级时因目录改名产生新的扩展实例。
...
同步后必须在 `chrome://extensions/` 点击 Later Space Collector 的 Reload，并确认显示版本 `1.9.3`。
...
7. **像素级打磨**：`28px`、居中 `+`、700ms 完成态、箭头 `0px / 0px` 中心误差、网页 CSS 污染修复。
